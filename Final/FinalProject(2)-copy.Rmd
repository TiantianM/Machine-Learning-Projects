...
$$X = WH$$
All the elements in these three matrics are non-negative. W represents the basis vectors in the reduced dimensions that are used to represent the original features, while H is the "loadings", which contains the weights that define the how each basis vector in W are constructed to represent the original feature set. 

In PCA, the loadings can be both negative and positive. In NMF, since the loadings and scores are always positive, which means that the contribution of each feature to the latent factors is always in the positive direction. Therefore, it's easier to interpret the loadings. The value of 0 means that the feature has no contribution of feature. A larger positive value means more contribution of feature on NMF dimension. Therefore, by looking at the value of the loadings, we are able to understand which features are important for the dimension and helps us better understand the data. 

Notes: When implementing NMF, we need to concern about our choice of factorization rank when doing NMF factorization. If the rank is too high, there may be overfitting issues because if including more vairables can better fit the data and reduce residuals. But if the rank is too small, the approximation could be bad for the data. So we need to be careful and critical when choosing the rank. 

The method of NMF can be applied in high dimensional data in different fields:
1. Image processing: Decompose images into parts and identify important patterns in images. 
2. Text mining: Identity main topics or themes of a set of documents.
3. Bioinformatics: Identity gene and molecular pattern.
4. Audio processing: Source separation, such as separating speech from music signal, and music analysis. 
...
```{r}
nmf <- nmf(data_biopsy, seq(2,6),seed = 123)
...
plot(nmf)
```
 The NMF plot illustrates the decomposition of the input data into a set of basis vectors and their corresponding coefficients. It visualize the relationships and patterns within the data. The cophenetic correlation measures the preservation of pairwise distances between original data points and their corresponding reconstructed data points. Higher cophenetic values indicate better preservation of the original data's structure.Dispersion represents the total distance between original data points and their approximations in the NMF model. Lower dispersion values suggest a better fit of the model to the data. The evar quantifies the proportion of total variance in the data explained by each component. Higher ever values indicate greater contribution of the component to the overall variance. Residuals measure the difference between the original data points and their reconstructions by the NMF model. Lower residuals indicate a better fit of the model to the data. RSS stands for Residual Sum of Squares, which quantifies the overall error between the original data and its approximation in the NMF model. Lower RSS values indicate better model fit. Silhouette coefficient measures the clustering quality of the NMF components. It assesses how well each sample fits within its assigned component compared to other components. Higher silhouette values suggest better-defined and distinct components. Spareness measures the degree of sparsity in the NMF model, indicating how many of the coefficients are close to zero. Higher spareness values suggest a more sparse representation, where only a few components contribute significantly to each data point.
 
  The plot likely displays these metrics for each number of components (ranging from 2 to 6) in the NMF model. By examining these metrics, we can assess the quality of the NMF results. We choose Elbow method to look for a point in the plot where the value of dispersion starts to level off or reaches a plateau. Selecting 3 components at this elbow point can be a reasonable choice, as it represents a balance between capturing sufficient information and avoiding overfitting.

#### Coefficient Matrix Heatmap
```{r}
nmf3 <- nmf(data_biopsy, rank =3, seed = 123)
H <- coef(nmf3)
...
heatmap(H, Colv = NA, Rowv = NA, 
        ...
        ylab = "Samples")
```
  The resulting heatmap plot will visualize the contribution of each component to each sample in the dataset. The intensity of the colors in the heatmap represents the magnitude of the coefficients, with darker colors indicating higher values and lighter colors indicating lower values. This plot show the relative importance and patterns of the components across the samples in the NMF analysis, with large coefficients on V6 and 1 sample, V1 and 2 samples, and V8 and 3 samples. Clusters of similar samples or features can be seen as blocks of similarly colored cells. Strong correlations between samples or features can be seen as diagonal lines of similarly colored cells.
