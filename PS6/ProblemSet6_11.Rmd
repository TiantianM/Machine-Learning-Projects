...
Yes, each time both of these two methods return the similar optimal M values. We decide to use 6 as the optimal M value.
...
rf_opt <- ranger(shares ~ ., data = combined, num.trees = 1500,
    mtry = 6, importance = "permutation")
```

```{r}
tdf <- data.table(Measure = rep("Permutation", 50), Variable = names(rf_opt$variable.importance),
    Importance = rf_opt$variable.importance)
...
```
```{r}
# Partial prediction for self_reference_avg_sharess
partial(object = rf_opt, pred.var = c("self_reference_avg_sharess"), grid.resolution = 25,
    plot = TRUE, rug = TRUE)
```
```{r}
# Partial prediction for self_reference_min_shares
partial(object = rf_opt, pred.var = c("self_reference_min_shares"), grid.resolution = 25,
    plot = TRUE, rug = TRUE)
```
```{r}
# Joint partial 
#partial(object = rf_opt, pred.var = c("self_reference_avg_sharess", #"self_reference_min_shares"),
#    grid.resolution = 20, plot = TRUE, rug = TRUE)
```
## Part 3 (30 pts)
...
Self_reference_avg_shares and self_reference_min_shares are top two important variables for both the random forest and the boosted tree, while the self_reference_min_shares is the most important for the boosted trees and the latter is the most important for the random forest. Also, in addition to these two variables, only LDA_02 and n_unique_tokens are in the top ten important variables for both methods. So we can see that the important variables are not really similar between the random forest and the boosted tree.
...
rf_preds <- predict(rf_opt, data = test)
rf_test_epe <- mean((rf_preds$predictions - test$shares)^2)

gbm_preds <- predict(gbm_opt, newdata = test)
gbm_test_epe <- mean((gbm_preds - test$shares)^2)
test_epe <- c("RF" = rf_test_epe, "GBM" = gbm_test_epe)
print(test_epe)
```
The optimal random forest performed better with a lower estimate of the EPE on the test set.