...
This homework covers an applied exercise related to validation sets and $K$-nearest neighbors regression.
...
```{r}
train <- read.csv("TaiwanPricing_TrainS2023.csv", header = T)
head(train)
test <- read.csv("TaiwanPricing_TestS2023.csv", header = T)
head(test)
```
...
knn_reg_kfold <- function(x, y, k, folds, seed){
  ...
  for (i in 1:folds){
    ...
    preds <- knn.reg(train_set,test_set,y,k)$pred
    mses[i]<- mean((preds-y)^2)
  }
...
k_e = data.frame()
for(k in 1:20){  
  knn_reg_kfold(X,y,k,2,123)
  e = c(k,kfold_estimate)
  k_e = rbind(k_e,e)
}
for(k in 1:20){  
  knn_reg_kfold(X,y,k,5,123)
  k_e$kfe2 = kfold_estimate
}
for(k in 1:20){  
  knn_reg_kfold(X,y,k,10,123)
  k_e$kfe3 = kfold_estimate
}
for(k in 1:20){  
  knn_reg_kfold(X,y,k,20,123)
  k_e$kfe4 = kfold_estimate
}
for(k in 1:20){  
  knn_reg_kfold(X,y,k,N,123)
  k_e$kfe5 = kfold_estimate
}

names(k_e)=c("k","kfe1","kfe2","kfe3","kfe4","kfe5")

ggplot()+
  geom_point(data = k_e, aes(x = k, y = kfe1,col = 2),,color = "blue") +
  geom_point(data = k_e, aes(x = k, y = kfe2),color = "pink")+
  geom_point(data = k_e, aes(x = k, y = kfe3),color = "red")+
  geom_point(data = k_e, aes(x = k, y = kfe4),color = "green")+
  geom_point(data = k_e, aes(x = k, y = kfe5),color = "yellow")+
...
for(k in 1:20){   
  mses[k] <- knn_reg_kfold(data.frame(train$Lat, train$Long),train$Price, k, 5, 50)
}


plot(1:20, mses, type="b", xlab = "Number of Nearest Neighbors", ylab = "MSE",
     main = "Expected Prediction Error for KNN Regression")
...
for(k in 1:20){   
  mses[k] <- knn_reg_kfold(data.frame(train$Lat, train$Long),train$Price, k, 50, 100)
}
...
![Example](ExampleLatLongMap.jpeg)
...
fig1 <- plot_ly(x = train$Lat, y = train$Long, z = preds_20, type = "contour",
               autocontour = TRUE, line = list(smoothing = 0.85),
...
fig2 <- plot_ly(x = train$Lat, y = train$Long, z = preds_1, type = "contour",
               autocontour = TRUE, line = list(smoothing = 0.85),
